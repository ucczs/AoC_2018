...
void initFrequencyTracker(FrequencyTracker *a, size_t initialSize)
{
...
bool checkFrequencyDouble(FrequencyTracker *a, int element)
{
...
bool insertFrequency(FrequencyTracker *a, int element)
{
    bool return_val = false;

    return_val = checkFrequencyDouble(a, element);
...
void printFrequencies(FrequencyTracker *a)
{
    for (int i = 0; i < a->used; i++)
    {
        printf("%d ", a->array[i]);
...
int main()
{
    int length_input = sizeof(input_data) / sizeof(input_data[0]);
    bool double_found = false;

    FrequencyTracker sumList;
    initFrequencyTracker(&sumList, 100);

    int sum = 0;
    while (double_found == false)
    {
        printf("\nNext round!\n");
        for (int i = 0; i < length_input; i++)
        {
            sum += input_data[i];
            double_found = insertFrequency(&sumList, sum);
