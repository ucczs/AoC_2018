...
void findWorstGuard_Strategy1(guardStatisticList *guardList, int *maxSleepGuardID, int *HighSleepingMinute)
{
    int maxSleep = 0;
...
void findWorstGuard_Strategy2(guardStatisticList *guardList, int *maxSleepGuardID, int *HighSleepingMinute)
{
    int worstMinCount = 0;

    for (int i = 0; i < guardList->used; i++)
    {
        guardStatistic currentGuard = guardList->array[i];
        for (int j = 0; j < RELEVANT_MINUTES; j++)
        {
            if(currentGuard.minuteStatistic[j] > worstMinCount)
            {
                worstMinCount = currentGuard.minuteStatistic[j];
                *maxSleepGuardID = currentGuard.guardID;
                *HighSleepingMinute = j;
            }
        }
    }
}
...
int main(void)
{
    ...
    findWorstGuard_Strategy1(&guardList, &mostSleepyGuardID, &highSleepingMinute);
    int sum_strategy1 = mostSleepyGuardID * highSleepingMinute;
    printf("Sum s1: %d\n", sum_strategy1);

    findWorstGuard_Strategy2(&guardList, &mostSleepyGuardID, &highSleepingMinute);
    int sum_strategy2 = mostSleepyGuardID * highSleepingMinute;
    printf("Sum s2: %d\n", sum_strategy2);
