...
void moveCart(scene_type* scene, int x_pos, int y_pos)
{
    ...
    case LEFT:
        ...
        {
            // collision
            scene->map[y_pos][x_pos-1].cart = NULL;
            scene->map[y_pos][x_pos].cart = NULL;
            scene->active_carts -= 2;
        }
        break;
    case UP:
        ...
        {
            // collision
            scene->map[y_pos-1][x_pos].cart = NULL;
            scene->map[y_pos][x_pos].cart = NULL;
            scene->active_carts -= 2;
        }
        break;
...
void printLastCart(scene_type* scene)
{
    for (int y = 0; y < Y_MAX; y++)
    {
        for (int x = 0; x < X_MAX; x++)
...
int main()
{
    ...
    scene_type scene;
    scene.tick_count = 0;
    scene.active_carts = CARTS_COUNT;

#if(TEST_RUN == 1)
    fp = fopen("D:\\Creativity\\Advent_of_Code\\AoC_2018\\13_C\\13_02\\test.txt", "r");
#else
    fp = fopen("D:\\Creativity\\Advent_of_Code\\AoC_2018\\13_C\\13_02\\input.txt", "r");
#endif
